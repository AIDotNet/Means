...
当前 `MeansLogDb` 是节点本地 WAL/snapshot 命名空间。`MetaSyncMode=Always` 会在对象可见前 flush + fsync WAL，保证本地崩溃恢复；但它不是 Raft/etcd 这类共享分布式 metadata。Console diagnostics 会暴露 `metadata.sharedNamespace=false` 和 `metadata.multiNodeWriteRisk`，当多个在线节点仍使用本地 metadata 时，生产环境不应把 S3 写入口负载均衡到多个节点。
...
默认 `DeterministicObjectPlacementPlanner` 使用 `Means:Cluster:PlacementSeed`、bucket、object key、version id 和 pool id 做确定性选择。未显式指定 pool 时，planner 会先为对象选择一个具备足够在线容量的 storage pool，然后只在该 pool 内选择 replica/EC shard 目标；显式指定 `PoolId` 时则严格限制在该 pool 内。

这个行为接近 MinIO server pool 的扩容模型：对象不会为了凑 quorum 自动跨多个 pool 混放。pool 内优先分散到不同故障域（`Means:Cluster:FaultDomain`，未配置时使用 node id），再分散到不同节点；当故障域或节点数量不足但磁盘数量满足时，再回退到同域/同节点不同磁盘。`Means:Storage:PlacementMinFaultDomains` 可把跨故障域从偏好提升为写入准入要求。
...
- `UploadPart` 优先按当前 EC profile 写入 part shards，EC 未启用时回退 full-copy part 文件，并记录 part metadata。
...
- XlFs 普通 `PutObject` 与 multipart `UploadPart` 已支持本地/跨节点对象级 `reed-solomon-v1` data/parity shards，并可在缺失 shard 时从剩余 shard 重建读取；后台 repair/rebalance 已有初版，生产级扩缩容、限速和故障域策略仍是后续项。
